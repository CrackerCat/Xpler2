...
open class Xpler2CompilerTask : DefaultTask() {
    ...
    @get:OutputDirectory
    lateinit var outputDirectory: Directory
    ...
    fun compiler() {
        ...
        XplerInitializeCache(sourceFile.absolutePath, xplerInitializeBean)
            .into(outputDirectory.asFile.absolutePath)
        println("XplerInitialize: $sourceFile -> ${Json.encodeToString(xplerInitializeBean)}")

        // build Xposed/Lspose configuration information
        XposedConfig.init(outputDirectory, xplerInitializeBean)
        LsposedConfig.init(outputDirectory, xplerInitializeBean)
    }
}