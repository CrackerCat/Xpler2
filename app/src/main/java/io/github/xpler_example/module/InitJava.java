...
public class InitJava {
    ...
    public static void init(XplerModuleInterface module) {
        try {
            ...
            if (!module.getPackageName().equals("com.tencent.mm")) return;// skip if not the target package
            ...
            Class<?> tinkerApplicationClazz = module.getClassLoader().loadClass("com.tencent.tinker.loader.app.TinkerApplication");
            ...
            module.hooker(onCreateMethod, new HookerCallback() {
                ...
                public void onBefore(@NonNull BeforeParams params) {
                    getModule().log("[Xpler2]Java -> onCreate called in TinkerApplication: " + params);
                }

                @Override
                public void onAfter(@NonNull AfterParams params) {
                    getModule().log("[Xpler2]Java -> TinkerApplication onCreate finished.");
                }
            });
        } catch (Throwable e) {
